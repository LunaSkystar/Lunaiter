...
class Qotd(commands.Cog):
    ...
    @commands.hybrid_command(name="qotd_suggest", description="Suggest a question for the Learners Unite daily question!")
    async def qotd_suggest(self, interaction: discord.Interaction, question: str): 
        try:
            staff_chat = await self.bot.fetch_channel(1044523629859311666)
    ...
    async def send_qotd(self):
        qotd_channel = self.bot.get_channel(1038052696143437874)
        conn = sqlite3.connect("lunaiter_data.db")
        c = conn.cursor()
